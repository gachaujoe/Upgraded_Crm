...
export default function App() {
  ...
  const handleAddIssue = async (e) => {
    e.preventDefault();
    const res = await fetch(`http://localhost:5000/clients/${selectedClientId}/issues`, {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({ title: issueTitle, description: issueDescription })
    });
    const updated = await res.json();
    setClients(prev => prev.map(c => c._id === selectedClientId ? updated : c));
    setIssueTitle('');
    setIssueDescription('');
  };

  const toggleSelect = id => setSelectedClientId(prev => prev === id ? null : id);
    ...
    <div className="min-h-screen bg-gray-100 p-4">
      ...
      <section className="mb-6">
        <form onSubmit={handleAddClient} className="flex gap-2">
          <input value={newClientName} onChange={e => setNewClientName(e.target.value)} placeholder="Name" required className="border p-2" />
          <input value={newClientEmail} onChange={e => setNewClientEmail(e.target.value)} placeholder="Email" className="border p-2" />
          <button type="submit" className="bg-blue-600 text-white p-2">Add Client</button>
        </form>
      </section>
      <section>
        {clients.map(client => (
          <div key={client._id} className="border p-4 mb-4">
            <div onClick={() => toggleSelect(client._id)} className="cursor-pointer flex justify-between items-center">
              <span>{client.name}</span>
              <span>{selectedClientId === client._id ? '▼' : '►'}</span>
            </div>
            {selectedClientId === client._id && (
              <div className="mt-2">
                ...
                <div className="mt-2">
                  <h4>Issues</h4>
                  {client.issues.length ? client.issues.map((i, idx) => (
                ...
                <form onSubmit={handleAddIssue} className="mt-2">
                  <input value={issueTitle} onChange={e => setIssueTitle(e.target.value)} placeholder="Issue Title" required className="border p-1 mb-1 w-full" />
                  <textarea value={issueDescription} onChange={e => setIssueDescription(e.target.value)} placeholder="Description" required className="border p-1 mb-1 w-full"></textarea>
                  <button type="submit" className="bg-green-600 text-white p-2">Add Issue</button>
                </form>
              </div>
            )}
          </div>
